  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (column) {
        ...
        await kanbanService.updateColumn(columnData)
      } else {
        addColumn({ ...columnData, id: crypto.randomUUID() })
        const label = title.toLowerCase().replace(/ /g, "-")
        await kanbanService.createColumn({ ...columnData, label: label })
      }
    } catch (error) {
      console.error("Error handling column:", error)
